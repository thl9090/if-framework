layui.config({
        ...
        cols: [[
            ...
            {field: 'title', title: '问题',
                templet: function(res) {
                    return '<a style="text-decoration:underline;cursor:pointer" href="'+server_url+'/layout/disclosure-index/often-que?port=1" target="_blank">' + res.title + '</a>';
                }},
            {field: 'status', title: '是否有效',templet: '<div>{{d.status === 1 ? "有效中" : "无效"}}</div>'},
            {field: 'opt', title: '操作', fixed: 'right', width: 200, align: 'center', toolbar: '#toolBar'}
        ]],
        url: 'sysCommonProblem/selectProblemsPage',
        method: 'post',
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            var index = layui.layer.open({
                title: "查看问题",
                type: 2,
                content: "commonProblem.html",
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回常见问题列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
                        });
                    }, 500)
                }
            ...
            var index = layui.layer.open({
                title: "编辑问题",
                type: 2,
                content: "commonProblem.html?id=2",
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回常见问题列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        } else if (layEvent === 'upline') { //下线
            var dataIds = [data.id];
            layer.confirm("您确定要发布吗？", {icon: 3, title: "确认"}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'sysCommonProblem/upline',
                    data: JSON.stringify(dataIds),
                    success: function (data) {
                        if (data.code == 200) {
                            layer.msg("操作成功", {icon: 1, time: 2000});
    ...
    $(".batchDel").click(function () {
        var checkStatus = table.checkStatus('developingCourseTable');
        if (checkStatus.data.length === 0) {
            layer.msg("请选择要删除的问题", {icon: 0, time: 2000});
            return;
        }
