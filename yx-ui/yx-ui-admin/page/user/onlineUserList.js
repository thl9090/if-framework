layui.config({
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        else if (layEvent === 'kickOut') { //强制下线
            var sessionIds = [data.sessionId];
            layer.confirm('您确定要下线该用户吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'user/kickOut',
