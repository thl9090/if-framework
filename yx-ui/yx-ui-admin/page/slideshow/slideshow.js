layui.config({
	...
	var base = layui.base,
        ...
        submitUrl = parent.pageOperation === 1?"sysSlideshow/add":"sysSlideshow/update";
    ...
    if(parent.pageOperation === 0 || parent.pageOperation === 2) {
        ...
        $.ajax({
            type: "GET",
            url: "sysSlideshow/select/"+parent.dataId,
            success: function(data){
                if(data.code==200){
                    ...
                    //下拉选项赋值
                    $("#clientType").val(rest.clientType);
                    form.render('select');
                }else{
    ...
    var uploadInst = upload.render({
        ...
        ,before: function(obj){
            ...
            obj.preview(function(index, file, result){
                $('#fileShow').empty();
                $('#fileShow').append('<img src="'+ result +'" alt="'+ file.name +'" class="layui-upload-img" style="width: 100px;height: 100px;margin-left: 10px;">');
            });
        }
        ,done: function(res){
            //上传成功
