layui.config({
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'isBorrower', title: '是否借款人',templet: '<div>{{d.isBorrower === 1 ? "是" : "否"}}</div>',width:100},
            {field: 'isDepositary', title: '是否受托人',templet: '<div>{{d.isDepositary === 1 ? "是" : "否"}}</div>',width:100},
	...
	$(".exportLogBtn").click(function () {
		...
		var index = layui.layer.open({
			...
			success: function (layero, index) {
				setTimeout(function () {
					layui.layer.tips('点击此处返回借款人列表', '.layui-layer-setwin .layui-layer-close', {
						tips: 3
					});
				}, 500)
			}
