layui.config({
    ...
    var tableIns = table.render({
        ...
        url: 'userInfo/selectBorrowerPage',
        method: 'post',
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
           ...
           /* $(window).resize(function () {
            ...
            layui.layer.full(index);
        } else if (layEvent === 'remove') { //移除借款人
            var dataIds = [data.id];
            layer.confirm('您确定要移除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'userInfo/cancelBorrower',
                    data: JSON.stringify(dataIds),
                    success: function (data) {
                        if (data.code == 200) {
                            if (data.data === true) {
        ...
        }else if (layEvent === 'select') { //选择借款人
            var oriBorrowerId=data.id;
            var oriBorrower=data.realName;

            parent.selectBorrowerCallBack(oriBorrowerId, oriBorrower);
    ...
    //设置原始借款人
    $(".batchOriBorrower").click(function () {
        var checkStatus = table.checkStatus('UserInfoTable');
        if (checkStatus.data.length === 0) {
            layer.msg("请选择借款人", {icon: 0, time: 2000});
            return;
        }
        var dataIds = [];
        var dataNames=[];
        for (var i = 0; i < checkStatus.data.length; i++) {
            dataIds[i] = checkStatus.data[i].id;
            dataNames[i]='['+checkStatus.data[i].realName+'-'+checkStatus.data[i].phone+']';
        }
        parent.selectBorrowerCallBack(dataIds, dataNames);
        var index = parent.layer.getFrameIndex(window.name);
        parent.layer.close(index);
