layui.config({
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'borrowUserName', title: '借款人',width:150},
            ...
            {field: 'opt', title: '操作', fixed: 'right', width: 180, align: 'center', toolbar: '#toolBar'}
        ]],
        url: 'product/selectBidingPage',
        method: 'post',
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            var index = layui.layer.open({
                ...
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回招标中标的列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
                        });
                    }, 500)
                }
           ...
           /* $(window).resize(function () {
            ...
            var dataIds = [data.id];
            layer.confirm("您确定要推送到首页吗？", {icon: 3, title: "确认"}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'product/push',
                    data: JSON.stringify(dataIds),
        ...
        }else if (layEvent === 'cancelPush') { //取消
            var dataIds = [data.id];
            layer.confirm("您确定要取消推送吗？", {icon: 3, title: "确认"}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'product/cancelPush',
                    data: JSON.stringify(dataIds),
                    success: function (data) {
                        if (data.code == 200) {
                            if (data.data === true) {
    ...
    $(".flowProduct_btn").click(function () {
        ...
        if (checkStatus.data.length === 0) {
            layer.msg("请选择要流标的数据", {icon: 0, time: 2000});
            return;
        }
