layui.config({
    ...
    var tableIns = table.render({
        ...
        url: 'sysRecruitment/selectRecruitmentPage',
        method: 'post',
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            var index = layui.layer.open({
                title: "查看招聘",
                type: 2,
                content: "recruitment.html",
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回招贤纳士列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
                        });
                    }, 500)
                }
    ...
    $(".batchDel").click(function () {
        ...
        if (checkStatus.data.length === 0) {
            layer.msg("请选择要删除的招聘", {icon: 0, time: 2000});
            return;
        }
