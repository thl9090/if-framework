layui.config({
    ...
    var tableIns = table.render({
        ...
        url: 'sysOpenAdvertising/selectPage',
        method: 'post',
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            var index = layui.layer.open({
                title: "查看开屏广告",
                type: 2,
                content: "openAdvertising.html",
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回开屏广告列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
                        });
                    }, 500)
                }
           ...
           /* $(window).resize(function () {
            ...
            layer.confirm("您确定要下线吗？", {icon: 3, title: "确认"}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'sysOpenAdvertising/downline',
                    data: JSON.stringify(dataIds),
                    success: function (data) {
                        if (data.code == 200) {
                            if (data.data === true) {
    ...
    $(".batchDel").click(function () {
        var checkStatus = table.checkStatus('openAdvertisingTable');
