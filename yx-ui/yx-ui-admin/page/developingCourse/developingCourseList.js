layui.config({
    ...
    var tableIns = table.render({
        ...
        url: 'sysDevelopingCourse/selectDevelopingCoursePage',
        method: 'post',
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            var index = layui.layer.open({
                title: "查看历程",
                type: 2,
                content: "developingCourse.html",
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回发展历程列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
                        });
                    }, 500)
                }
            ...
            layer.confirm('您确定要删除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'sysDevelopingCourse/delete',
            ...
            var index = layui.layer.open({
                title: "编辑历程",
                type: 2,
                content: "developingCourse.html?id=2",
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回发展历程列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
            ...
            layer.confirm("您确定要下线吗？", {icon: 3, title: "确认"}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'sysDevelopingCourse/downline',
                    data: JSON.stringify(dataIds),
                    success: function (data) {
                        if (data.code == 200) {
            ...
            layer.confirm("您确定要发布吗？", {icon: 3, title: "确认"}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'sysDevelopingCourse/upline',
                    data: JSON.stringify(dataIds),
                    success: function (data) {
                        if (data.code == 200) {
                            layer.msg("操作成功", {icon: 1, time: 2000});
    ...
    $(".batchDel").click(function () {
        ...
        if (checkStatus.data.length === 0) {
            layer.msg("请选择要删除的历程", {icon: 0, time: 2000});
            return;
        }
