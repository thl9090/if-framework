layui.config({
    ...
    //从父页面取到导出类型
    var exportType=parent.exportType;
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            if(data.status===1){
                ...
                location.href='/exportLog/downloadExcel/'+data.id;
            }else{
                layer.msg("正在处理中，请等待完成后下载", {icon: 0, time: 2000});
                return;
            }
        }
    });
