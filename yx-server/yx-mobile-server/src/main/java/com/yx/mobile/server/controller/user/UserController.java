package com.yx.mobile.server.controller.user;
...
//            userInfoVO.setFddStatusStr("");//不需要弹窗提醒
//            userInfoVO.setFddTip(0);//不需要弹框提示
//            userInfoVO.setFddStatus(0);//认证菜单不可点击
//        }
    ...
    @ApiOperation(value = "修改邮箱--验证",
            notes = "传参说明：{paramKey:从浏览器地址栏获取}")
    @PostMapping("/updateEmailValidate")
    @SysLogOpt(module = "修改邮箱--验证", value = "修改邮箱--验证", operationType = Constants.LogOptEnum.UNKNOW)
    public ResultModel updateEmailValidate(UserEmailUpdate model, HttpServletRequest request) throws Exception {

//        userBusinessService.updateEmailValidate(model);
        return ResultUtil.ok();
    }
