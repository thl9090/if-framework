...
public class SysActivityMsgController extends BaseController {
    ...
    public ResultModel selectPage(RequestParam model, HttpServletRequest request) {
        ...
        sysSlideshow.setClientType(ClientTypeEnum.APP.getStatus());
        ...
        for (SysSlideshow record : records) {
            ...
            if (tmpUrl.length() > 0
                ...
                tmpUrl.append(tmpUrl.toString().indexOf("?") != -1 ? "&" : "?")
                        .append("channelType=")
                        .append(clientInfo.getChannelType())
                        .append("&channelVersion=")
                        .append(clientInfo.getChannelVersion());
            }
