...
public class SysContentController extends BaseController {
	...
	public ResultModel selectRegistereAgreement() {
		...
		//移动端
		sysContent.setClientType(ClientTypeEnum.PC.getStatus());
	...
	public ResultModel selectCommonProblemPage(RequestParam model) {
		...
		Page page1 = sysContentService.selectVOPage(page, sysContent);
		return ResultUtil.ok(page1);
	}


	@ApiOperation(value = "常见问题--预览", notes = "参数：{currentPage:当前页}")
	@PostMapping("/selectCommonProblemPagePreview")
	@SysLogOpt(module = "常见问题--预览", value = "常见问题--预览", operationType = Constants.LogOptEnum.QUERY)
	public ResultModel selectCommonProblemPagePreview(RequestParam model) {
		Page page=new Page();
		if(model.getCurrentPage()!=null){
			page.setCurrent(model.getCurrentPage());
		}
