package com.yx.web.server.controller.user;
...
//        sysSmsService.sendCodeMessage(model.getPhone(), Long.valueOf(smscode));
        ...
        CacheUtil.getCache().set(Constants.CacheNamespaceEnum.USER_SEND_SMS_COUNT.value()+model.getPhone(),1,60);
        CacheUtil.getCache().set(Constants.CacheNamespaceEnum.SMS_CAPTCHA.value() + captchaId, smscode, 2 * 60);
        CacheUtil.getCache().set(Constants.CacheNamespaceEnum.SMS_CAPTCHA_PHONE.value() + captchaId, model.getPhone(), 2 * 60);
    ...
    public ResultModel info(HttpServletRequest request) {
        ...
        if (userInfoVO.getRegisterPhone() != null) {
            String registerPhoneStr = userInfoVO.getRegisterPhone().toString();
            if (registerPhoneStr.length() > 10) {
                registerPhoneStr = registerPhoneStr.substring(0, 3) + "****" + registerPhoneStr.substring(registerPhoneStr.length() - 4, registerPhoneStr.length());
                userInfoVO.setRegisterPhoneStr(registerPhoneStr);
            }
        }
        ...
        String loginTimeStr = (String) CacheUtil.getCache().get(Constants.CacheNamespaceEnum.USER_LOGIN_TIME.value() + userInfoVO.getId());
        userInfoVO.setLoginTimeStr(StringUtils.isBlank(loginTimeStr) ? DateUtils.formatDateTime(new Date()) : loginTimeStr);
...
//            }else{
//                //认证通过
//                if((fddUser.getStatus().intValue()== FddStatusEnum.STEP_2.getId().intValue())||
//                        (fddUser.getStatus().intValue()==FddStatusEnum.STEP_3.getId().intValue())||
//                        (fddUser.getStatus().intValue()==FddStatusEnum.STEP_4.getId().intValue())){
//
//                    userInfoVO.setFddStatusStr("已认证");
//                    userInfoVO.setFddTip(0);//需要弹框提示
//                    userInfoVO.setFddStatus(0);//认证菜单不可点击
//                }else if(fddUser.getStatus().intValue()==FddStatusEnum.STEP_5.getId().intValue()){
//                    userInfoVO.setFddStatusStr("认证中");
//                    userInfoVO.setFddTip(0);//需要弹框提示
//                    userInfoVO.setFddStatus(0);//认证菜单不可点击
//                }else if(fddUser.getStatus().intValue()==FddStatusEnum.STEP_6.getId().intValue()){
