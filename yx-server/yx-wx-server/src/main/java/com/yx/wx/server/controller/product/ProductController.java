package com.yx.wx.server.controller.product;
...
//            }
    ...
    public ResultModel gmvCount() throws Exception {
        Map<String, Object> data = new HashMap<>();
        Object o = CacheUtil.getCache().get(Constants.CacheNamespaceEnum.PLATFORM_COUNT_MONEY.value());
        Object o1 = CacheUtil.getCache().get(Constants.CacheNamespaceEnum.USER_COUNT_MONEY.value());
        if(o==null||o1==null){
...
//                    receiptService.getTotalInterest();
            ...
            CacheUtil.getCache().set(Constants.CacheNamespaceEnum.PLATFORM_COUNT_MONEY.value(),AmountNoPointSerializer.format(platformCountMoney),10*60);
            CacheUtil.getCache().set(Constants.CacheNamespaceEnum.USER_COUNT_MONEY.value(),AmountSerializer.format(userContMoney),10*60);
        }else{
            String platformCountMoney =String.valueOf(o);
            String userContMoney = String.valueOf(o1);
            data.put("platformCountMoney", platformCountMoney);
            data.put("userContMoney",  userContMoney);
        }
